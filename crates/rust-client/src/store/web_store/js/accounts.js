import { accountCodes, accountStorages, accountVaults, accountAuths, accounts, foreignAccountCode, } from "./schema";
// GET FUNCTIONS
export async function getAccountIds() {
    try {
        ...
        if (error instanceof Error) {
            ...
            console.error("Failed to retrieve account IDs: Unexpected value thrown: ", String(error));
        }
        throw error;
    }
}
export async function getAllAccountHeaders() {
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error("Error fetching all latest account headers:", error.toString());
        }
        else {
            console.error("Error fetching all latest account headers: Unexpected value thrown:", String(error));
        }
        throw error;
    }
}
export async function getAccountHeader(accountId) {
    ...
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error(`Error fetching account header for ID ${accountId}:`, error.toString());
        }
        else {
            console.error(`Error fetching account header for ID ${accountId}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
export async function getAccountHeaderByCommitment(accountCommitment) {
    ...
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error(`Error fetching account header for commitment ${accountCommitment}:`, error.toString());
        }
        else {
            console.error(`Error fetching account header for commitment ${accountCommitment}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
export async function getAccountCode(codeRoot) {
    ...
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error(`Error fetching account code for root ${codeRoot}:`, error.toString());
        }
        else {
            console.error(`Error fetching account code for root ${codeRoot}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
export async function getAccountStorage(storageRoot) {
    ...
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error(`Error fetching account storage for root ${storageRoot}:`, error.toString());
        }
        else {
            console.error(`Error fetching account storage for root ${storageRoot}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
export async function getAccountAssetVault(vaultRoot) {
    ...
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error(`Error fetching account vault for root ${vaultRoot}:`, error.toString());
        }
        else {
            console.error(`Error fetching account vault for root ${vaultRoot}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
...
var ACCOUNT_AUTH_MAP = new Map(); // Added types for Map
export async function fetchAndCacheAccountAuthByPubKey(pubKey) {
    ...
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error(`Error fetching account auth for pubKey ${pubKey}:`, error.toString());
        }
        else {
            console.error(`Error fetching account auth for pubKey ${pubKey}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
...
export async function insertAccountCode(codeRoot, code) {
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error(`Error inserting code with root: ${codeRoot}:`, error.toString());
        }
        else {
            console.error(`Error inserting code with root: ${codeRoot}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
export async function insertAccountStorage(storageRoot, storageSlots) {
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error(`Error inserting storage with root: ${storageRoot}:`, error.toString());
        }
        else {
            console.error(`Error inserting storage with root: ${storageRoot}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
export async function insertAccountAssetVault(vaultRoot, assets) {
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error(`Error inserting vault with root: ${vaultRoot}:`, error.toString());
        }
        else {
            console.error(`Error inserting vault with root: ${vaultRoot}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
export async function insertAccountRecord({ id: accountId, codeRoot, storageRoot, vaultRoot, nonce, committed, accountSeed, commitment, }) {
    try {
        ...
        if (error instanceof Error) {
            ...
            console.error(`Error inserting account: ${accountId}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
export async function insertAccountAuth(pubKey, secretKey) {
    try {
        ...
        if (error instanceof Error) {
            console.error(`Error inserting account auth for pubKey: ${pubKey}:`, error.toString());
        }
        else {
            console.error(`Error inserting account auth for pubKey: ${pubKey}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
...
export async function getForeignAccountCode(accountIds) {
    try {
        ...
        if (error instanceof Error) {
            ...
            console.error("Error fetching foreign account code: Unexpected value thrown:", String(error));
        }
        throw error;
    }
}
export async function lockAccount(accountId) {
    try {
        ...
        if (error instanceof Error) {
            ...
            console.error(`Error locking account: ${accountId}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
...
export async function undoAccountStates(accountCommitments) {
    try {
        ...
        // Add unknown type
        if (error instanceof Error) {
            console.error(`Error undoing account states: ${accountCommitments}:`, error.toString());
        }
        else {
            console.error(`Error undoing account states: ${accountCommitments}: Unexpected value thrown:`, String(error));
        }
        throw error;
    }
}
