...
use miden_lib::account::components::rpo_falcon_512_library;
...
fn generate_known_procedure_roots() -> std::io::Result<()> {
    ...
    let auth_rpo_proc_root = rpo_falcon_512_library()
        .mast_forest()
        .procedure_digests()
        .next()
        .expect("AuthRpoFalcon512 component should have one procedure root");

    writeln!(f, "/// Procedure root of the `AuthRpoFalcon512` authentication component.")?;
    writeln!(f, "pub const AUTH_RPO_FALCON_512: &str =\n    \"{auth_rpo_proc_root}\";")?;
